var scriptReplacedLink = "", validationErrorCount = 0, validatersvrdstring1 = "test", validatersvrdstring2 = "test", selectedRow = null, timeConverted = "", hours = 0, minutes = 0, suffix = "AM", hoursU = 0, minutesU = 0, suffixU = "PM", hoursE = 0, minutesE = 0, suffixE = "AM", i = 0, j = 0, tabledatastring = "table_data_export=failed;", tempstring = "", tableexist = "";
var today = new Date();
...
function onEdit(td) {
    ...
    if (suffixE == "PM" && hoursE != 12) {
        hoursE = Number(hoursE) + 12;
    }

    if (suffixE == "AM" && hoursE == 12) {
        hoursE = "00";
    }

    document.getElementById("Time").value = hoursE + ":" + minutesE;
    //alert(hoursE + ":" + minutesE);
    //we made some changes here hey there!
}
...
function validate() {
    ...
    validatersvrdstring1 = document.getElementById("Subject").value;
    validatersvrdstring2 = document.getElementById("Link").value;
    //alert(validatersvrdstring);
    //alert(validatersvrdstring.indexOf("C-C"));
    if (validatersvrdstring2.indexOf("-C") != -1 || validatersvrdstring2.indexOf("C-") != -1 || validatersvrdstring2.indexOf("-R") != -1 || validatersvrdstring2.indexOf("R-") != -1 || validatersvrdstring1.indexOf("-C") != -1 || validatersvrdstring1.indexOf("C-") != -1 || validatersvrdstring1.indexOf("-R") != -1 || validatersvrdstring1.indexOf("R-") != -1 || validatersvrdstring2.indexOf("R-C") != -1 || validatersvrdstring2.indexOf("C-R") != -1 || validatersvrdstring1.indexOf("R-C") != -1 || validatersvrdstring1.indexOf("C-R") != -1 || validatersvrdstring1.indexOf("C-C") != -1 || validatersvrdstring1.indexOf("R-R") != -1 || validatersvrdstring2.indexOf("C-C") != -1 || validatersvrdstring2.indexOf("R-R") != -1) {
        if (validationErrorCount >= 3) {

            while (document.getElementById("Link").value.indexOf("-C") != -1 || document.getElementById("Link").value.indexOf("C-") != -1 || document.getElementById("Link").value.indexOf("-R") != -1 || document.getElementById("Link").value.indexOf("R-") != -1 || document.getElementById("Subject").value.indexOf("-C") != -1 || document.getElementById("Subject").value.indexOf("C-") != -1 || document.getElementById("Subject").value.indexOf("-R") != -1 || document.getElementById("Subject").value.indexOf("R-") != -1 || document.getElementById("Link").value.indexOf("R-C") != -1 || document.getElementById("Link").value.indexOf("C-R") != -1 || document.getElementById("Subject").value.indexOf("R-C") != -1 || document.getElementById("Subject").value.indexOf("C-R") != -1 || document.getElementById("Subject").value.indexOf("C-C") != -1 || document.getElementById("Subject").value.indexOf("R-R") != -1 || document.getElementById("Link").value.indexOf("C-C") != -1 || document.getElementById("Link").value.indexOf("R-R") != -1) { //alert("munna working"); }
                document.getElementById("Subject").value = document.getElementById("Subject").value.replace("C-R", "C R").replace("R-C", "R C").replace("C-C", "C C").replace("R-R", "R R").replace("-C", "C").replace("C-", "C").replace("-R", "R").replace("R-", "R");
                document.getElementById("Link").value = document.getElementById("Link").value.replace("-C", "%2DC").replace("C-", "C%2D").replace("-R", "%2DR").replace("R-", "R%2D").replace("C-R", "C%2DR").replace("R-C", "R%2DC").replace("C-C", "C%2DC").replace("R-R", "R%2DR");
            }
            // r---r =(replace) r-r
            // while true (since d.g./l.v.i(r-r) != -1 /contains r-r/) 
            //r-r =(replace) r r
            isValid = true;
            ...
            alert("reserved string!\n\nYou're not allowed to use reserved string such as -C, C-, -R, R-, C-R, R-C, C-C, R-R");
            isValid = false;
